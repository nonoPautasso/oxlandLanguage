...
namespace Assets.Scripts.Levels.Vowels
{
	public class VowelsController : LevelController
	{
		...
		public void SubmitLetter (Button bubble)
		{
			...
			if (index != -1) {
				view.SetBubbleCorrect (bubble);
				view.PlayRightSound ();
				// Clicked letter is a vowel
				view.ShowLetter (index, bubble.GetComponentInChildren<Text> ().text);    // Tells view to show the letter at appropriate index
			...
			if (model.GetNumRevealedLetters () == 5)
